...
def load_smart_image(name):
    # 扫描所有可能的存放目录
    for folder in ["images", "Images", "图片"]:
        if os.path.exists(folder):
            all_files = os.listdir(folder)
            for f in all_files:
                # 剥离后缀后匹配纯文件名，并去除两端空格
                pure_name = os.path.splitext(f)[0].strip()
                if pure_name == name.strip():
                    st.image(f"{folder}/{f}", use_container_width=True)
                    return True
    return False
...
if st.session_state.step == 0:
    ...
    if st.button("A. 好开心又能近距离接触代老师了"): st.session_state.force_result = "易子雨路"; st.session_state.step = 5; st.rerun()
    ...
    if st.button("B. 卖保险去了 管你"): st.session_state.scores["邓晶"] += 1; st.session_state.step = 7; st.rerun()
...
# --- 6. 结果结算 ---
    ...
    if not load_smart_image(winner):
        st.warning(f"⚠️ 图片未找到: images/{winner}.jpg")
